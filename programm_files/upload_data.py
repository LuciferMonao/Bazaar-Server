...
for number, product in enumerate(get_product_list()):
    ...
    print(colorama.Fore.RED,f"main", colorama.Fore.RESET, f"Created Object for item {product}, {number}/{len(get_product_list())}")
while True:
    for number, item in enumerate(products):
        print(colorama.Fore.RED, f"main", colorama.Fore.RESET, f"Refreshing price of {item.productID}, {number}/{len(products)}")
        item.refresh_price()
    ...
    for waited in range(WAIT): time.sleep(1); print(colorama.Fore.RED, "main", colorama.Fore.RESET, f"{waited}/{WAIT}")
